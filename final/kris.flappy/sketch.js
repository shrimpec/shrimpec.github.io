...
let flappyFont, gameFont, explosion, jump, score, hurt, music, countd

function preload() {
    bgImage = loadImage('graphics/background1.png')
    ...
    countd = loadSound('sounds/countd.wav')
...
function title() {
    ...
    textAlign(CENTER)
    textFont(flappyFont)
    text("Flappy Bird", width / (2 * 1.74), 100)
    textSize(14)
...
function play() {
    ...
    if (spawnTimer > 2) {
        ...
        pipe.y = constrain(lastY + random (-50, 50), 100, 200)
        lastY = pipe.y
        pipes.push(pipe)
        spawnTimer = 0
    }
